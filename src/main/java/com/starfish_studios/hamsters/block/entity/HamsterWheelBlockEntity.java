...
public class HamsterWheelBlockEntity extends BlockEntity implements IAnimatable {
    ...
    private <E extends HamsterWheelBlockEntity> PlayState controller(final AnimationEvent<E> event) {
        assert level != null;
        BlockPos blockPos = this.getBlockPos();
        if (HamsterWheelBlock.isOccupied(level, blockPos)) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.sf_nba.hamster_wheel.spin").loop("animation.sf_nba.hamster_wheel.spin"));
