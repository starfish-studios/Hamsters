...
public class SearchForItemsGoal extends Goal {
    ...
    public boolean canUse() {
        if (mob.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty() && !mob.isInSittingPose()) {
            List<ItemEntity> list = mob.getLevel().getEntitiesOfClass(ItemEntity.class, mob.getBoundingBox().inflate(horizontalSearchRange, verticalSearchRange, horizontalSearchRange), itemEntity -> ingredient.test(itemEntity.getItem()));
